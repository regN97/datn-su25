    ...
    public function showReport(Request $request)
{
    try {
        ...
        $currentUserShift = UserShift::with('workShift')
        ...
        $bankRevenue = $bills->where('payment_status_id', 2)
            ->whereIn('payment_method', ['card', 'bank_transfer', 'credit_card'])
            ->sum('total_amount');
        ...
        $duration = Carbon::parse($currentUserShift->check_in)->diff($endTime)->format('%H giờ %I phút');
        ...
        $shiftReport = [
            'shift' => [
                ...
                'shift_description' => $currentShift->description ?? 'Không có mô tả',
    ...
    public function endShift(Request $request)
    {
        try {
            ...
            // Gọi API closeSession từ POSController
            $response = app(\App\Http\Controllers\Cashier\POSController::class)->closeSession(
                new Request([
            ...
            // Kiểm tra phản hồi từ closeSession
            if ($response->getStatusCode() !== 200) {
                Log::error('Lỗi khi đóng ca từ ShiftReportController.', [
                    'user_id' => $user->id,
                    'response' => $response->getContent(),
                ]);
                return response()->json(['message' => 'Có lỗi khi đóng ca làm việc.'], 422);
            }
    ...
    public function history(Request $request)
{
    try {
        ...
        $userShifts = UserShift::with('workShift')
            ->where('user_id', $user->id)
        ...
        $shiftHistory = $userShifts->map(function ($userShift) {
            ...
            return [
                ...
                'duration' => Carbon::parse($userShift->check_in)->diff($userShift->check_out)->format('%H giờ %I phút'),
                'total_revenue' => $totalRevenue,
                'cash_revenue' => $cashRevenue,
                'bank_revenue' => $bankRevenue,
