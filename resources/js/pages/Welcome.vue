    ...
    <div
        ...
        <nav class="flex items-center justify-end gap-4">
            <template v-if="$page.props.auth?.user">
                <Link :href="route($page.props.auth.user.role_id === 3 ? 'cashier.dashboard' : 'admin.dashboard')"
            ...
            <template v-else>
                <div class="relative">
                    <button @click="toggleLoginOptions"
                        class="inline-flex items-center rounded-sm border border-[#19140035] px-5 py-1.5 text-sm leading-normal text-[#1b1b18] hover:border-[#1915014a] dark:border-[#3E3E3A] dark:text-[#EDEDEC] dark:hover:border-[#62605b]
                               focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 focus:ring-offset-white
                               transition-all duration-200 ease-in-out hover:scale-105 hover:shadow-sm">
                        Đăng nhập
                        <svg class="ml-2 -mr-0.5 h-4 w-4 transform transition-transform duration-200 ease-in-out"
                    ...
                    >
                        <div v-if="showLoginOptions" @click.away="closeLoginOptions"
                            class="absolute right-0 mt-2 w-48 rounded-md shadow-lg bg-white dark:bg-[#2D2D2A] ring-1 ring-black ring-opacity-5 focus:outline-none z-10 origin-top-right">
                            <div class="py-1" role="menu" aria-orientation="vertical" aria-labelledby="options-menu">
                                <Link :href="route('login')"
                                    class="block px-4 py-2 text-sm text-[#1b1b18] dark:text-[#EDEDEC] hover:bg-gray-100 dark:hover:bg-[#3E3E3A]
