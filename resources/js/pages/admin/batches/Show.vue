            ...
            <div class="receipt-container">
                ...
                <div class="receipt-summary" v-if="currentBatch">
                    <p>
                        ...
                        <span>{{ formatCurrency(aggregatedProducts.reduce((sum, item) => sum + item.total_amount, 0)) }}</span>
                    </p>
                    <p>
                        <strong>Giảm giá:</strong> <span>-{{ formatCurrency(currentBatch.discount_amount || 0) }}</span>
                    </p>
                    <p>
                        <strong>Phí vận chuyển:</strong> <span>{{ formatCurrency(currentBatch.shipping_fee || 0) }}</span>
                    </p>
                    <p class="total-line">
                        <strong>Tổng tiền cần trả:</strong> <span>{{ formatCurrency(remainingAmount + (currentBatch?.paid_amount || 0)) }}</span>
                    </p>
                    <p>
                        <strong>Đã trả:</strong> <span>{{ formatCurrency(currentBatch.paid_amount) }}</span>
                    </p>
                    <p>
                        <strong>Còn phải trả:</strong> <span>{{ formatCurrency(remainingAmount) }}</span>
                    </p>
                </div>
            ...
            <div class="space-y-4 p-4">
                ...
                <div>
                    ...
                    <input
                        type="text"
                        disabled
                        :value="props.batch[0]?.purchase_order?.po_number"
                        class="h-10 w-full rounded-md border border-gray-300 px-3 outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-500"
                    />
                </div>
            </div>
