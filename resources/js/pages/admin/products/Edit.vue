        ...
        <div class="container mx-auto max-w-5xl mt-8">
            <div class="bg-white rounded-xl shadow p-8">
                <div class="mb-6 flex items-center justify-between">
                    ...
                    <Link href="/admin/products" class="px-4 py-2 rounded bg-gray-200 hover:bg-gray-300 text-gray-700">Quay lại</Link>
                </div>
                <form @submit.prevent="submit">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                        ...
                        <div class="space-y-4">
                            <div>
                                ...
                                <input v-model="form.name" type="text" class="w-full border rounded px-3 py-2" placeholder="Nhập tên sản phẩm" />
                            </div>
                            <div>
                                <label class="font-semibold block mb-1">Mã SKU *</label>
                                <input v-model="form.sku" type="text" class="w-full border rounded px-3 py-2" placeholder="Nhập mã SKU" />
                                <div class="text-xs text-gray-400 mt-1">Ví dụ: G7-MTHH-001</div>
                            </div>
                            <div class="flex gap-4">
                                <div class="flex-1">
                                    <label class="font-semibold block mb-1">Giá nhập *</label>
                                    <input v-model="form.purchase_price" type="number" min="0" class="w-full border rounded px-3 py-2" />
                                </div>
                                <div class="flex-1">
                                    <label class="font-semibold block mb-1">Giá bán *</label>
                                    <input v-model="form.selling_price" type="number" min="0" class="w-full border rounded px-3 py-2" />
                                </div>
                            </div>
                            <div>
                            ...
                            <div class="flex gap-4">
                                <div class="flex-1">
                                    ...
                                    <input v-model="form.min_stock_level" type="number" min="0" class="w-full border rounded px-3 py-2" />
                                </div>
                                <div class="flex-1">
                                    <label class="font-semibold block mb-1">Tồn kho tối đa</label>
                                    <input v-model="form.max_stock_level" type="number" min="0" class="w-full border rounded px-3 py-2" />
                                </div>
                            </div>
                            <div>
                                <label class="font-semibold block mb-1">Mô tả</label>
                                <textarea v-model="form.description" class="w-full border rounded px-3 py-2" rows="3" placeholder="Mô tả về sản phẩm"></textarea>
                                <div class="text-xs text-gray-400 mt-1 text-right">{{ form.description.length }}/5000 ký tự</div>
                            </div>
                        </div>
                        <!-- Cột phải -->
                        <div class="space-y-4">
                            <div>
                                ...
                                <div v-if="form.image_type === 'url'">
                                    <input v-model="form.image_url" type="text" class="w-full border rounded px-3 py-2" placeholder="Nhập URL ảnh sản phẩm" />
                                </div>
                                <div v-else>
                                    <input type="file" @change="e => form.image_file = e.target.files[0]" class="w-full border rounded px-3 py-2" />
                                </div>
                            </div>
                            <div>
                            ...
                            <div>
                                ...
                                <div class="flex gap-2">
                                    <input v-model="form.barcode" type="text" class="flex-1 border rounded px-3 py-2" placeholder="Nhập hoặc tạo mã vạch" />
                                    <button type="button" class="bg-purple-500 text-white px-3 py-2 rounded hover:bg-purple-600 transition">Tạo ngẫu nhiên</button>
                                </div>
                            </div>
                            <div>
                    ...
                    <div class="flex justify-end gap-2 mt-8">
                        <Link href="/admin/products" class="px-6 py-2 rounded bg-gray-200 hover:bg-gray-300 text-gray-700">Quay lại</Link>
                        <button type="submit" class="px-8 py-2 rounded bg-gradient-to-r from-purple-500 to-pink-500 text-white font-semibold hover:from-purple-600 hover:to-pink-600 transition">Lưu</button>
                    </div>
                </form>
            </div>
        </div>
